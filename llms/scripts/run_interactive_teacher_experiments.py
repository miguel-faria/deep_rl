...
args = (" --data-dir %s --cache-dir %s --train-filename %s --test-filename %s --val-filename %s --results-path %s --task %s --student-model %s --teacher-model %s"
		...
		% (DATASET_DIR, CACHE_DIR, TRAIN_FILE, TEST_FILE, VALIDATION_FILE, RESULTS_FILE, TASK, STUDENT_MODEL, TEACHER_MODEL, MAX_TOKENS, N_BEAMS, N_SAMPLES,
		   MAX_STUDENT_SAMPLES, MM_TYPE, INTERVENE_BEHAVIOUR, INTERVENTION_UTILITY, TEACHER_EXPLANATION, STUDENT_EXPLANATION, INTERVENTION_THRESH))
