...
def load_models(rng_seed: int, train_data: pd.DataFrame, num_samples: int, student_model_path: str, teacher_model_path: str, task: str, use_explanations: bool, student_expl_type: str,
	...
	if model_lib == 'hf':
		...
		student_gen_model = LLM(student_model_path, gpu_memory_utilization=0.75, enforce_eager=False)
		...
		if use_explanations:
			...
			if student_expl_type.find('human') != -1:
				...
				teacher_gen_model = LLM(teacher_model_path, gpu_memory_utilization=0.75, enforce_eager=False)
