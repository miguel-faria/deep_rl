...
def train_mountain_car(wandb_run: wandb.run):
	...
	buffer = ReplayBuffer(10000, env.observation_space, env.action_space, "cuda" if use_gpu else "cpu", handle_timeout_termination=False)
	rng_gen = np.random.default_rng(1245)
	obs, *_ = env.reset()
	sample_actions = env.action_space.sample()
	ddpg.init_networks(1245, obs, sample_actions, actor_lr=3e-4, critic_lr=3e-4)
	init_eps = 1.0
	final_eps = 0.05
	warmup = 200
	train_freq = 2
	...
	for it in range(N_ITERATIONS):
		...
		avg_actor_loss = 0
		episod_start = epoch
		eps = eps_update(1, init_eps, final_eps, 0.5, it, max_steps=400)
		while not done:
			...
			if epoch >= warmup:
				if epoch % train_freq == 0:
					critic_loss, agent_loss = jax.device_get(ddpg.update_models(buffer, 128, 0.9, None))
