...
class ReuptationBasedStudent(StudentModel):
		...
		if self._utility_type.find('least') != -1:
			...
			if self._reputation_type.find('weight') != -1:
				...
				for teacher_model in self._teachers:
					...
					teacher_scores = teacher_model.predict_confidence(sample, with_explanation=False)
	...
	def predict(self, sample: Dict, expl: Union[List, str] = None, intervene: bool = False) -> Tuple[str, str]:
		pass
