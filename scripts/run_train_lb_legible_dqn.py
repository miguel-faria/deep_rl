...
for i in range(args.limits[0], args.limits[1] + 1):
	...
	N_SPAWN_FOODS = i
	args = (" --n-agents %d --n-layers %d --buffer %d --gamma %f --beta %f --layer-sizes %s --reward %s --iterations %d --max-cycles %d --batch %d --train-freq %d "
			...
			% (N_AGENTS, N_LAYERS, BUFFER, GAMMA, BETA, ' '.join([str(x) for x in LAYERS]),	LEG_REWARD,																# DQN parameters
			   ...
			   N_PLAYERS, PLAYER_LEVEL, FIELD_LENGTH, N_FOODS, FOOD_LVL, STEPS_EPISODE, N_SPAWN_FOODS,																# Environment parameters
	...
	args += ((" --dueling" if USE_DUELING else "") + (" --ddqn" if USE_DDQN else "") + (" --render" if USE_RENDER else "") + ("  --gpu" if USE_GPU else "") +
			 ...
			 (" --debug" if DEBUG else "") + (" --use-opt-vdn" if OPT_VDN else "") + (" --n-leg-agents %d" % N_LEG_AGENTS) )
