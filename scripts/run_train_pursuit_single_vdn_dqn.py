...
N_REQUIRED_HUNTER = 2
FIELD_LENGTH = 10
STEPS_EPISODE = 500 * N_PREYS
WARMUP_STEPS = STEPS_EPISODE * 2
USE_RENDER = False
USE_TARGETS = False
TRAIN_TARGETS = ['prey_1']

args = (" --nagents %d --nlayers %d --buffer %d --gamma %f --layer-sizes %s --iterations %d --cycles %d --batch %d --train-freq %d "
		...
		% (N_AGENTS, N_LAYERS, BUFFER, GAMMA, ' '.join([str(x) for x in LAYERS]),																		# DQN parameters
		   N_ITERATIONS, CYCLES, BATCH_SIZE, TRAIN_FREQ, TARGET_FREQ, ALPHA, TAU, INIT_EPS, FINAL_EPS, EPS_DECAY, EPS_TYPE, WARMUP_STEPS, CYCLE_EPS,	# Train parameters
