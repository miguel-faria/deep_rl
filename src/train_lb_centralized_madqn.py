...
def convert_joint_act(action: int, num_agents: int) -> np.ndarray:
...
def main():
	...
	for loc in food_locs:
		...
		if restart_train:
			...
			central_madqn.load_model(restart_info[1], model_path.parent.absolute() / restart_info[0])
		...
		for cycle in cycles_range:
			...
			if debug:
				...
				Path.mkdir(model_path, parents=True, exist_ok=True)
				central_madqn.save_model(('food_%dx%d_cycle_%d' % (loc[0], loc[1], cycle + 1)), model_path)
				sys.stdout.flush()

		print('Saving final model')
		central_madqn.save_model(('food_%dx%d' % (loc[0], loc[1])), model_path)
		sys.stdout.flush()
		...
		while not game_over:
			...
			history += [get_history_entry(obs, actions, central_madqn.num_agents)]
			obs = next_obs
